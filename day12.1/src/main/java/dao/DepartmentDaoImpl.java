...
public class DepartmentDaoImpl implements DepartmentDao {
	...
	@Override
	public String deleteDepartment(String dept) {
		String mesg = "Department Deletion failed!";
		String jpql = "select d from Department d where d.name=:name";
		// 1. get session from SF
		Session session = getFactory().getCurrentSession();
		// 2. Begin a tx
		...
		try {
			Department selectedDept = session.createQuery(jpql, Department.class).setParameter("name", dept)
					.getSingleResult();
			session.remove(selectedDept);

			tx.commit();
			mesg = "Department Deletion Succesfull!";
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
			throw e;
